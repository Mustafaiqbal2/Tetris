...
///////////////////////////////////////////////////////
void fallingPiece(float& timer, float& delay, int& colorNum, int&z, int& y_disp, int point_3[][2], int point_4[][2],float K,int& n,int next_point[][2],int& nextColor)
{
	//int z;
    if (timer>delay)
    {
...
bool destruction(int point_4[][2], int& bombColor,int colorNum)
{
	bool flag=false;
...
void falling_bomb(int point_4[][2], float& b_timer,double& b_delay, int& bomb_rand, bool& bomb_prob, int& bombColor, int colorNum)
{
	static int z=0;
	if (b_timer>b_delay)
	{
	...
	if(!bomb_limit(point_4) || destruction(point_4, bombColor, colorNum))
	{
		for(int i=0;i<4;i++)
		{
			point_4[i][0]=-10;

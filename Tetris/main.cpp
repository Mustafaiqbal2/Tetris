...
//BACK GROUND RANDOMIZED
	...
	buffer.loadFromFile("sounds/tetris.wav");  //MUSIC LOADED INTO BUFFER
//DECLARATION OF TEXTURE CLASS OBJECTS
	...
	switch(rnd_back)
	{
		...
		case 2:
			...
			break;
		case 3:
			obj2.loadFromFile("img/background3.png");
			break;
		case 4:
			obj2.loadFromFile("img/background4.png");
			break;
	}
	obj3.loadFromFile("img/frame.png");
	obj4.loadFromFile("img/pause_menu.png");
    obj5.loadFromFile("img/main_menu.png");
    obj6.loadFromFile("img/help.png");
    obj7.loadFromFile("img/hs.png");
    obj8.loadFromFile("img/shadow.png");
...
//ALL INITIAL VARIABLE DECLARED
    int delta_x=0, colorNum=1, bombColor=1,level=1,score=0,lines,lineCount=0, nextColor=1+rand()%7;
    float timer=0, delay=0.3,b_timer=0,K=0.3,pitch=0;
    double b_delay=0.3;
    bool rotate=0, leave=false, leave2=false, leave3=true,leave4=true,click1=false,click2=false,click3=false,click4=false,click5=false, overlap=true,bomb_prob=false;
    Clock clock;
    int n=rand()%7;
    int highS;
...
//VARIABLES REDECALRED FOR ANOTHER GAME IF USER WISHES
		int delta_x=0, colorNum=1, bombColor=1,level=1,score=0,lines,lineCount=0;
  		float timer=0, delay=0.3,b_timer=0,K=0.3,pitch=0;
  		double b_delay=0.3;
  		bool rotate=0, leave=false, leave2=false,leave3=false,space=false,bomb_prob=false;
...
////////////////////////////////////////
		...
		while(window.pollEvent(f))
		{
			...
			{
				if (g.mouseButton.button == sf::Mouse::Left) 	//CONFIRMS IF LEFT MOUSE BUTTON WAS PRESSED
				{
			...
			if (f.type == Event::KeyPressed || click1 || click2|| click3|| click4|| click5)//CHECKS IF KEY IS PRESSED OR MOUSE HAS BEEN CLICKED
			{
				...
				if(((f.key.code == Keyboard::Num1)||((Keyboard::isKeyPressed(Keyboard::Num1))))||!leave4 || click1) //IF NUM 1 KEY is PRESSED OR OBJECT HAS BEEN CLICKED RETURNS TRUE
				{
					...
					//---Event Listening Part---//
						...
						fallingPiece(timer, delay, colorNum, z, y_disp, point_3,point_4,K,n,next_point, nextColor);               //Example: fallingPiece() function is called here
						...
						if(!bomb_limit(point_4))//IF BOMB LIMIT HAS BEEN REACHED CHECKS NEW BOMB PROBALITY
						{
							...
							bomb_prob=0;
						falling_bomb(point_4, b_timer, b_delay,bomb_rand, bomb_prob, bombColor,colorNum);    //DROPS BOMB 
						//destruction(point_4, bombColor,colorNum);
